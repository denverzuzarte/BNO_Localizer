...
int main()
{
	...
	float accel_y;
	float accel_z;
	uint8_t acc_x_0, acc_x_1, acc_x_2, acc_x_3;
	uint8_t acc_y_0, acc_y_1, acc_y_2, acc_y_3;
	uint8_t acc_z_0, acc_z_1, acc_z_2, acc_z_3;
	uint8_t xor;
	float acc_pkt[16];
	uint32_t tmp;
	uint8_t msg_id = 0x01;
		...
		memcpy(&tmp, &accel_x, 0x04);
		tmp >>= 4;	
		acc_x_0 = (tmp >> 21) & (0x7f);
		acc_x_1 = (tmp >> 14) & (0x7f);
		acc_x_2 = (tmp >>  7) & (0x7f);
		acc_x_3 = (tmp >>  0) & (0x7f);
		memcpy(&tmp, &accel_y, 0x04);
		tmp >>= 4;
		acc_y_0 = (tmp >> 21) & (0x7f);
                acc_y_1 = (tmp >> 14) & (0x7f);
                acc_y_2 = (tmp >>  7) & (0x7f);
                acc_y_3 = (tmp >>  0) & (0x7f);
		memcpy(&tmp, &accel_z, 0x04);
		tmp >>= 4;
                acc_z_0 = (tmp >> 21) & (0x7f);
                acc_z_1 = (tmp >> 14) & (0x7f);
                acc_z_2 = (tmp >>  7) & (0x7f);
		...
		xor = 0x0c^ acc_x_0^ acc_x_1^ acc_x_2^ acc_x_3^
                      acc_y_0^ acc_y_1^ acc_y_2^ acc_y_3^
                      acc_z_0^ acc_z_1^ acc_z_2^ acc_z_3^ msg_id;

		putchar_raw(0xF1);putchar_raw(msg_id);putchar_raw(0x0c);
		putchar_raw(acc_x_0); putchar_raw(acc_x_1); putchar_raw(acc_x_2); putchar_raw(acc_x_3);
		putchar_raw(acc_y_0); putchar_raw(acc_y_1); putchar_raw(acc_y_2); putchar_raw(acc_y_3);
		putchar_raw(acc_z_0); putchar_raw(acc_z_1); putchar_raw(acc_z_2); putchar_raw(acc_z_3);
		putchar_raw(0xF2);
		stdio_flush();
        }
}
