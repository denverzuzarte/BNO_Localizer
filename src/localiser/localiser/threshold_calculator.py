...
#   zupt_var     (float) - ZUPT measurement noise variance (default 1e-3); scales R_zupt
...
class ThresholdCalculatorNode(Node):
    def __init__(self):
        super().__init__('threshold_calculator')
        ...
        self.get_logger().info(
            f'Phase 1/{self._still_duration}s — keep IMU flat and STILL.'
        )
    ...
    def _end_still_phase(self) -> None:
        ...
        a_norms2 = np.array([np.dot(a, a) for a in self._still_accel])
        w_norms2 = np.array([np.dot(w, w) for w in self._still_gyro])
        self._sigma_a     = math.sqrt(float(np.mean(a_norms2)))
        self._sigma_omega = math.sqrt(float(np.mean(w_norms2)))

        self.get_logger().info(
            f'Still phase done.  sigma_a={self._sigma_a:.6f}  sigma_omega={self._sigma_omega:.6f}\n'
            f'Phase 2/{self._walk_duration}s — START WALKING normally.'
        )
    ...
    def _save_plot(self, T_still: np.ndarray, T_walk: np.ndarray, gamma: float, fallback: bool) -> None:
        ...
        # gradient bars — intensity proportional to count within each dataset
    ...
    def _end_walk_phase(self) -> None:
        ...
        if fallback:
            # no clear bimodal separation — fall back to static P99 × 5
        ...
        self.get_logger().info(
            f'\n'
            f'=== SHOE Threshold Calculator Results ===\n'
            ...
            f'\n'
            f'  Stance windows classified (T < gamma) : {len(T_stance)}\n'
            f'  Swing  windows classified (T >= gamma): {len(T_swing)}\n'
            f'\n'
            f'  Paste into calibration.json:\n'
            f'    "sigma_a":     {self._sigma_a},\n'
            f'    "sigma_omega": {self._sigma_omega},\n'
            f'    "zupt_gamma":  {gamma},\n'
            f'    "zupt_var":    0.001\n'
        )
